function [ss] = ray_trace(filename, antPosSTA, antposition)
    ...
    % ss = sigstrength(RXs, tx, pm, "map", "FirstFloor.stl"); %接收机,发射机,传播模型,地图
    ss = sigstrength(RXs, tx, pm, "map", filename); %接收机,发射机,传播模型,地图
    
    % save('indoor_fingerprint_result.mat', 'ss', 'antPosSTA');
end
